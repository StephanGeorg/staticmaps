...
StaticMaps.prototype._drawLines = function () {
  ...
  return new Promise(_.bind(function(resolve, reject) {
    ...
    processArray(this.lines, _.bind(this.__draw, this))
...
StaticMaps.prototype.__draw = function (line) {
  ...
  return new Promise(_.bind(function(resolve, reject) {
      ...
      var points = line.coords.map(_.bind(function(coord){
        return [
      ...
      baseImage.getBuffer(Jimp.AUTO, _.bind(function (err,result) {
        ...
        if (type === 'polyline') {
          ...
          gm(result)
            ...
            .drawPolyline(points)
            .toBuffer(_.bind(function(err, buffer) {
              if (err) reject(err);
              Jimp.read(buffer, _.bind(function(err,image){
                if (err) reject(err);
...
StaticMaps.prototype._drawMarker = function () {
  ...
  return new Promise (_.bind(function (resolve,reject){
    ...
    this.markers.forEach(_.bind(function(marker){
...
StaticMaps.prototype._loadMarker = function () {
  ...
  return new Promise (_.bind(function (resolve,reject){
    ...
    this.markers.forEach(_.bind(function (icon){
...
function _simplify (points, tolerance) {
  ...
  tolerance = tolerance || 11;

  if (!points || !points.length) return;

  var new_coords = [points[0]];

  for (var x=0; x<points.length; x++) {
    var last = _.last(new_coords);
    var dist = Math.sqrt(Math.pow(last[0] - points[x], 2) + Math.pow(last[1] - points[1], 2));
    if (dist > tolerance) new_coords.push(points[x]);
  }

  new_coords.push(_.last(points));
